...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.bg='';//background (Ogge and such)
		...
		Game.goldenCookie.choose=function()
		{
			...
			if (Game.goldenCookie.wrath>0 && Math.random()<0.3) list.push('blood frenzy','chain kart');
			else if (Math.random()<0.01 && Game.cookiesEarned>=100000) list.push('chain kart');
		...
		Game.goldenCookie.click=function()
		{
			if (Game.goldenCookie.life>0)
			{
				...
				if (Game.goldenClicks>=1) Game.Win('Golden kart');
				if (Game.goldenClicks>=7) Game.Win('Lucky kart');
				...
				if (Game.goldenCookie.chain>0) choice='chain kart';
				Game.goldenCookie.last=choice;
				
				if (choice!='chain kart') Game.goldenCookie.chain=0;
				if (choice=='frenzy')
				{
					...
					Game.frenzyPower=7;
					Game.recalculateGains=1;
					Game.Popup('Frenzy : kart production x7 for '+time+' seconds!');
				}
				else if (choice=='multiply cookies')
				{
					var moni=Math.min(Game.cookies*0.1,Game.cookiesPs*60*20)+13;//add 10% to cookies owned (+13), or 20 minutes of kart production - whichever is lowest
					Game.Earn(moni);
					Game.Popup('Lucky! +'+Beautify(moni)+' cookies!');
				}
				else if (choice=='ruin cookies')
				{
					var moni=Math.min(Game.cookies*0.05,Game.cookiesPs*60*10)+13;//lose 5% of cookies owned (-13), or 10 minutes of kart production - whichever is lowest
				...
				else if (choice=='blood frenzy')
				{
					...
					Game.recalculateGains=1;
					Game.Popup('Elder frenzy : kart production x666 for '+time+' seconds!');
				}
				else if (choice=='clot')
				{
					...
					Game.recalculateGains=1;
					Game.Popup('Clot : kart production halved for '+time+' seconds!');
				}
		...
		Game.UpdateMenu=function()
		{
			...
			if (Game.onMenu=='log')
			{
				...
				// '<div class="title">31/08/2013 - too many Ogge</div>'+
			...
			else if (Game.onMenu=='stats')
			{
				...
				'<div class="subsection">'+
				'<div class="title">General</div>'+
				'<div class="listing"><b>Kart in bank :</b> <div class="price plain">'+Beautify(Game.cookies)+'</div></div>'+
				'<div class="listing"><b>Total Kart (all time) :</b> <div class="price plain">'+Beautify(Game.cookiesEarned)+'</div></div>'+
				...
				'<div class="listing"><b>Kart per second :</b> '+Beautify(Game.cookiesPs,1)+' <small>(multiplier : '+Beautify(Math.round(Game.globalCpsMult*100),1)+'%)</small></div>'+
				'<div class="listing"><b>Kart per click :</b> '+Beautify(Game.computedMouseCps,1)+'</div>'+
				'<div class="listing"><b>Kart clicks :</b> '+Beautify(Game.cookieClicks)+'</div>'+
				'<div class="listing"><b>Hand-made Kart :</b> '+Beautify(Game.handmadeCookies)+'</div>'+
				'<div class="listing"><b>Golden Kart clicks :</b> '+Beautify(Game.goldenClicks)+'</div>'+//' <span class="hidden">(<b>Missed golden cookies :</b> '+Beautify(Game.missedGoldenClicks)+')</span></div>'+
				...
				'<div class="listing"><small>(Note : each heavenly chip grants you +2% KpS multiplier. You can gain more chips by resetting with a lot of cookies.)</small></div>'+
				'<div class="listing"><div class="icon" style="background-position:'+(-19*48)+'px '+(-7*48)+'px;"></div> <span style="vertical-align:100%;"><span class="title" style="font-size:22px;">'+Game.prestige['Heavenly chips']+' heavenly chip'+(Game.prestige['Heavenly chips']==1?'':'s')+'</span> (+'+(Game.prestige['Heavenly chips']*2)+'% KpS)</span></div>'):'')+
		...
		Game.getNewTicker=function()
		{
			...
			{				
				if (Game.Objects['Ogge'].amount>0) list.push(choose([
				'<q>Kommunism är en grupp närbesläktade ideer om att produktionsmedlen i ett samhälle är gemensamt ägda.</q><sig>ogge</sig>',
				'<q>Polisen presenterar en lägesuppdatering kring arbetet med operation "Rimfrost" i Malmö..</q><sig>ogge</sig>',
				'<q>Idiot!</q><sig>ogge</sig>',
				'<q>Har du ett problem ällär?</q><sig>ogge</sig>',
				]));

				if (Game.Objects['Albin Olsson'].amount>0) list.push(choose([
					'<q>Jag är weeb!</q><sig>Albin Olsson</sig>',
					]));
				
				if (Game.Objects['Ogge'].amount>=50) list.push(choose([
					'<q>Absolutely disgusting.</q><sig>ogge</sig>',
					'<q>Ogge är SK Elvans mest ambitiösa spelare!.</q><sig>ogge</sig>',
					'<q>Ogge är SK Elvans mest ambitiösa spelare!</q><sig>ogge</sig>',
					'<q>Ogge är, sett till hur många karts han tillverkar, värdelös.</q><sig>ogge</sig>',
					'<q>Ogge bänkar 160kg (slak).</q><sig>ogge</sig>',
					'<q>Ogge har 43 sidechicks!</q><sig>ogge</sig>',
					'<q>Ogge föddes inuti biltema!.</q><sig>ogge</sig>'
				]));
				
				if (Game.Objects['Albin Olsson'].amount>50) list.push(choose([
					'<q>Nu jävlar blir det hårdrock!</q><sig>Albin Olsson</sig>',
				]));
				if (Game.HasAchiev('Just wrong')) list.push(choose([
				'News : cookie manufacturer downsizes, sells own grandmother!',
				'<q>It has betrayed us, the filthy little thing.</q><sig>ogge</sig>',
				'<q>It tried to get rid of us, the nasty little thing.</q><sig>ogge</sig>',
				'<q>It thought we would go away by selling us. How quaint.</q><sig>ogge</sig>',
				'<q>I can smell your rotten kart.</q><sig>ogge</sig>'
				]));
				
				if (Game.Objects['Ogge'].amount>=1 && Game.pledges>0 && Game.elderWrath==0) list.push(choose([
				'<q>shrivel</q><sig>ogge</sig>',
				'<q>writhe</q><sig>ogge</sig>',
				'<q>throb</q><sig>ogge</sig>',
				'<q>gnaw</q><sig>ogge</sig>',
				'<q>We will rise again.</q><sig>ogge</sig>',
				'<q>A mere setback.</q><sig>ogge</sig>',
				'<q>We are not satiated.</q><sig>ogge</sig>',
				'<q>Too late.</q><sig>ogge</sig>'
				]));
				
				if (Game.Objects['Kommunism'].amount>0) list.push(choose([
				'News : Är en krigshandling av Zlatan mot MFF!',
				'News : Emilia, 20, saknad i sju dagar – utreds som kidnappning!',
				'News : Fick kebabsås på pizzan – misshandlade bagaren!',
				'News : Kommer bli krig om inte Säpo ingriper.',
				'News : Lööfs val - Jämlikhet eller Åkesson som statsminister.'
				]));
				
				if (Game.Objects['Theo'].amount>0) list.push(choose([
				'News : JUST NU: Kvinna skadad i biljakt!',
				'News : kart factories involved in chocolate weather controversy!',
				'News : kart factories on strike, robotic minions employed to replace workforce!',
				'News : kart factories on strike - workers demand to stop being paid in cookies!',
				'News : factory-made kart linked to obesity, says study.'
				]));
				...
				if (Game.Objects['Starkeman'].amount>0) list.push(choose([
				...
				'News : "explain to me again why we need particle accelerators to make kart?" asks misguided local woman.',
				'News : first starkeman successfully turned on, doesn\'t rip apart reality!',
			...
			if (list.length==0)
			{
				if (Game.cookiesEarned<5) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<50) list.push('Jag gillar kart!');
				else if (Game.cookiesEarned<100) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<500) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<1000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<3000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<6000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<10000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<20000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<30000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<40000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<60000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<80000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<100000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<200000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<300000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<450000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<600000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<1000000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<5000000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<10000000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<30000000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<100000000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<300000000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<1000000000) list.push('Jag gillar kart!.');
				else if (Game.cookiesEarned<10000000000) list.push('Jag gillar kart!. Success!<br><span style="font-size:50%;">(you win a Kart)</span>');
				else if (Game.cookiesEarned<10100000000) list.push('Jag gillar kart!.');//only show this for 100 millions (it's funny for a moment)
			}
		...
		Game.Object=function(name,commonName,desc,pic,icon,background,price,cps,drawFunction,buyFunction)
		{
			...
			// 	var str='';
			// 	if(name == 'Ogge'){
			// 			str+='var audio1 = new Audio(\''+ name +'.mp3\');audio1.play();'
			// 	}
			// 	return str;
			// }
		...
		new Game.Object('Ogge','Ogge|Ogge|made','A nice ogge to make kart.','grandma','grandmaIcon','grandmaBackground',100,function(){
			var mult=0;
			if (Game.Has('Farmer Ogge')) mult++;
			if (Game.Has('Worker Ogge')) mult++;
			if (Game.Has('Miner Ogge')) mult++;
			if (Game.Has('Cosmic Ogge')) mult++;
			if (Game.Has('Transmuted Ogge')) mult++;
			if (Game.Has('Altered Ogge')) mult++;
			if (Game.Has('Grandmas\' Ogge')) mult++;
			if (Game.Has('AntiOgge')) mult++;
			if (Game.Has('Bingo center/Research facility')) mult+=2;
			if (Game.Has('Ritual rolling pins')) mult++;
			var add=0;
			if (Game.Has('One mind')) add+=Game.Objects['Ogge'].amount*0.02;
			if (Game.Has('Communal brainsweep')) add+=Game.Objects['Ogge'].amount*0.02;
			...
			var list=['grandma'];
			if (Game.Has('Farmer Ogge')) list.push('grandma');
			if (Game.Has('Worker Ogge')) list.push('grandma');
			if (Game.Has('Miner Ogge')) list.push('grandma');
			if (Game.Has('Cosmic Ogge')) list.push('grandma');
			if (Game.Has('Transmuted Ogge')) list.push('grandma');
			if (Game.Has('Altered Ogge')) list.push('grandma');
			if (Game.Has('Grandmas\' Ogge')) list.push('grandma');
			if (Game.Has('AntiOgge')) list.push('grandma');
			return choose(list);
		...
		Game.Objects['Ogge'].sellFunction=function()
		{
			Game.Win('Just wrong');
			if (this.amount==0)
			{
...
Game.Objects['Albin Olsson'].sellFunction=function()
{
    ...
    if (this.amount==0)
    {
		...
		new Game.Object('Kommunism','kommunistiskt samhälle|kommunistiska samhällen|klar','Grows work from people.','farm','farmIcon','farmBackground',500,function(){
			return Game.ComputeCps(2,Game.Has('Cheap hoes')*0.5,Game.Has('Fertilizer')+Game.Has('Cookie trees')+Game.Has('Genetically-modified kart'));
		},Game.NewDrawFunction(0,16,16,64,2,32),function(){
			if (this.amount>=1) Game.Unlock(['Cheap hoes','Fertilizer']);if (this.amount>=10) Game.Unlock('Cookie trees');if (this.amount>=50) Game.Unlock('Genetically-modified kart');
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Ogge'].amount>0) Game.Unlock('Farmer Ogge');
			if (this.amount>=1) Game.Win('Kommunism');if (this.amount>=50) Game.Win('Reap what you sow');if (this.amount>=100) Game.Win('Farm ill');
		});
		new Game.Object('Theo','theo|theo|made','Theo är väldigt farlig! Produces large quantities of kart.','theo','theoIcon','grandmaBackground',3000,function(){
			...
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Ogge'].amount>0) Game.Unlock('Worker Ogge');
		...
		new Game.Object('Mine','mine|mines|mined','Mines out cookie dough and chocolate chips.','mine','mineIcon','mineBackground',10000,function(){
			...
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Ogge'].amount>0) Game.Unlock('Miner Ogge');
		...
		new Game.Object('Shipment','shipment|shipments|shipped','Brings in fresh kart from the kart planet.','shipment','shipmentIcon','shipmentBackground',40000,function(){
			...
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Ogge'].amount>0) Game.Unlock('Cosmic Ogge');
		...
		new Game.Object('Alchemy lab','alchemy lab|alchemy labs|transmuted','Turns gold into kart!','alchemylab','alchemylabIcon','alchemylabBackground',200000,function(){
			...
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Ogge'].amount>0) Game.Unlock('Transmuted Ogge');
		...
		new Game.Object('Portal','portal|portals|retrieved','Opens a door to the Kartverse.','portal','portalIcon','portalBackground',1666666,function(){
			...
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Ogge'].amount>0) Game.Unlock('Altered Ogge');
		...
		new Game.Object('Time machine','time machine|time machines|recovered','Brings kart from the past, before they were even eaten.','timemachine','timemachineIcon','timemachineBackground',123456789,function(){
			...
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Ogge'].amount>0) Game.Unlock('Grandmas\' Ogge');
		...
		new Game.Object('Starkeman','Starkeman|Starkeman|condensed','Starkeman är väldigt stark! Condenses the antimatter in the universe into kart.','linus','linusIcon','antimattercondenserBackground',3999999999,function(){ 
			...
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Ogge'].amount>0) Game.Unlock('AntiOgge');
			if (this.amount>=1) Game.Win('Antibatter');if (this.amount>=50) Game.Win('Quirky quarks');if (this.amount>=100) Game.Win('It does matter!');
		});
		// Game.Objects['Starkeman'].displayName='<span style="font-size:65%;">Starkeman</span>';//shrink the name since it's so large
		
		new Game.Object('Don Beh','Don Beh|Don Beh|made','Don Beh har inget användbart syfte.','donbeh','donbehIcon','grandmaBackground',69999999999,function(){
			var mult=0;
			var add=0;
			return Game.ComputeCps(0,0,0);
		},Game.NewDrawFunction(function(){
			var list=['donbeh'];
			//slumpmässig figur
			return choose(list);
		...
		new Game.Upgrade('Reinforced index finger','The mouse gains <b>+1</b> kart per click.<br>Cursors gain <b>+0.1</b> base KpS.<q>prod prod</q>',100,[0,0]);
		...
		new Game.Upgrade('Thousand fingers','The mouse and cursors gain <b>+0.1</b> kart for each non-cursor object owned.<q>clickity</q>',500000,[0,6]);
		new Game.Upgrade('Million fingers','The mouse and cursors gain <b>+0.5</b> kart for each non-cursor object owned.<q>clickityclickity</q>',50000000,[1,6]);
		new Game.Upgrade('Billion fingers','The mouse and cursors gain <b>+2</b> kart for each non-cursor object owned.<q>clickityclickityclickity</q>',500000000,[2,6]);
		new Game.Upgrade('Trillion fingers','The mouse and cursors gain <b>+10</b> kart for each non-cursor object owned.<q>clickityclickityclickityclickity</q>',5000000000,[3,6]);
		
		order=200;
		new Game.Upgrade('Forwards from Ogge','Ogge gain <b>+0.3</b> base KpS.<q>RE:RE:thought you\'d get a kick out of this ;))</q>',Game.Objects['Ogge'].basePrice*tier1,[1,0]);
		new Game.Upgrade('Steel-plated rolling pins','Ogge are <b>twice</b> as efficient.',Game.Objects['Ogge'].basePrice*tier2,[1,0]);
		new Game.Upgrade('Lubricated dentures','Ogge are <b>twice</b> as efficient.<q>Squish</q>',Game.Objects['Ogge'].basePrice*tier3,[1,1]);
		
		order=300;
		new Game.Upgrade('Cheap hoes','Kommunism gain <b>+0.5</b> base KpS.',Game.Objects['Kommunism'].basePrice*tier1,[2,0]);
		new Game.Upgrade('Fertilizer','Kommunism are <b>twice</b> as efficient.<q>It\'s people, I swear.</q>',Game.Objects['Kommunism'].basePrice*tier2,[2,0]);
		new Game.Upgrade('Cookie trees','Kommunism are <b>twice</b> as efficient.<q>A relative of the breadfruit.</q>',Game.Objects['Kommunism'].basePrice*tier3,[2,1]);
		
		order=400;
		new Game.Upgrade('Sturdier conveyor belts','Factories gain <b>+4</b> base KpS.',Game.Objects['Theo'].basePrice*tier1,[4,0]);
		new Game.Upgrade('Child labor','Factories are <b>twice</b> as efficient.<q>Cheaper, healthier workforce - and so much more receptive to whipping! Nice</q>',Game.Objects['Theo'].basePrice*tier2,[4,0]);
		new Game.Upgrade('Sweatshop','Factories are <b>twice</b> as efficient.<q>Slackers will be terminated.</q>',Game.Objects['Theo'].basePrice*tier3,[4,1]);
		
		order=500;
		new Game.Upgrade('Sugar gas','Mines gain <b>+10</b> base KpS.<q>A pink, volatile gas, found in the depths of some chocolate caves.</q>',Game.Objects['Mine'].basePrice*tier1,[3,0]);
		...
		order=600;
		new Game.Upgrade('Vanilla nebulae','Shipments gain <b>+30</b> base KpS.',Game.Objects['Shipment'].basePrice*tier1,[5,0]);
		...
		order=700;
		new Game.Upgrade('Antimony','Alchemy labs gain <b>+100</b> base KpS.<q>Actually worth a lot of mony.</q>',Game.Objects['Alchemy lab'].basePrice*tier1,[6,0]);
		...
		order=800;
		new Game.Upgrade('Ancient tablet','Portals gain <b>+1,666</b> base KpS.<q>A strange slab of peanut brittle, holding an ancient cookie recipe. Neat!</q>',Game.Objects['Portal'].basePrice*tier1,[7,0]);
		...
		order=900;
		new Game.Upgrade('Flux capacitors','Time machines gain <b>+9,876</b> base KpS.<q>Bake to the future.</q>',1234567890,[8,0]);
		...
		order=20000;
		new Game.Upgrade('Kitten helpers','You gain <b>more KpS</b> the more milk you have.<q>meow may I help you</q>',9000000,[1,7]);
		new Game.Upgrade('Kitten workers','You gain <b>more KpS</b> the more milk you have.<q>meow meow meow meow</q>',9000000000,[2,7]);
		...
		order=200;new Game.Upgrade('Prune juice','Ogge are <b>twice</b> as efficient.<q>Gets me going.</q>',Game.Objects['Ogge'].basePrice*tier4,[1,2]);
		order=300;new Game.Upgrade('Genetically-modified cookies','Farms are <b>twice</b> as efficient.<q>All-natural mutations.</q>',Game.Objects['Kommunism'].basePrice*tier4,[2,2]);
		order=400;new Game.Upgrade('Radium reactors','Factories are <b>twice</b> as efficient.<q>Gives your cookies a healthy glow.</q>',Game.Objects['Theo'].basePrice*tier4,[4,2]);
		...
		order=20000;
		new Game.Upgrade('Kitten engineers','You gain <b>more KpS</b> the more milk you have.<q>meow meow meow meow, sir</q>',9000000000000,[3,7]);
		...
		order=250;
		new Game.Upgrade('Farmer Ogge','Ogge are <b>twice</b> as efficient.',Game.Objects['Kommunism'].basePrice*tier2,[10,9],function(){Game.Objects['Ogge'].drawFunction();});
		new Game.Upgrade('Worker Ogge','Ogge are <b>twice</b> as efficient.',Game.Objects['Theo'].basePrice*tier2,[10,9],function(){Game.Objects['Ogge'].drawFunction();});
		new Game.Upgrade('Miner Ogge','Ogge are <b>twice</b> as efficient.',Game.Objects['Mine'].basePrice*tier2,[10,9],function(){Game.Objects['Ogge'].drawFunction();});
		new Game.Upgrade('Cosmic Ogge','Ogge are <b>twice</b> as efficient.',Game.Objects['Shipment'].basePrice*tier2,[10,9],function(){Game.Objects['Ogge'].drawFunction();});
		new Game.Upgrade('Transmuted Ogge','Ogge are <b>twice</b> as efficient.',Game.Objects['Alchemy lab'].basePrice*tier2,[10,9],function(){Game.Objects['Ogge'].drawFunction();});
		new Game.Upgrade('Altered Ogge','Ogge are <b>twice</b> as efficient.',Game.Objects['Portal'].basePrice*tier2,[10,9],function(){Game.Objects['Ogge'].drawFunction();});
		new Game.Upgrade('Ogges\' Ogge','Ogge are <b>twice</b> as efficient.',Game.Objects['Time machine'].basePrice*tier2,[10,9],function(){Game.Objects['Ogge'].drawFunction();});
		...
		new Game.Upgrade('Bingo center/Research facility','Ogge-operated science lab and leisure club.<br>Ogge are <b>4 times</b> as efficient.<br><b>Regularly unlocks new upgrades</b>.',100000000000,[11,9],function(){Game.SetResearch('Specialized chocolate chips');});
		...
		new Game.Upgrade('Ritual rolling pins','[Research]<br>Ogge are <b>twice</b> as efficient.<q>The result of years of scientific research!</q>',40000000000,[2,9],function(){Game.SetResearch('Underworld ovens');});
		...
		new Game.Upgrade('One mind','[Research]<br>Each Ogge gains <b>+1 base KpS for each 50 Ogge</b>.<div class="warning">Note : the grandmothers are growing restless. Do not encourage them.</div><q>We are one. We are many.</q>',160000000000,[4,9],function(){Game.elderWrath=1;Game.SetResearch('Exotic nuts');});
		...
		new Game.Upgrade('Communal brainsweep','[Research]<br>Each Ogge gains another <b>+1 base KpS for each 50 Ogge</b>.<div class="warning">Note : proceeding any further in scientific research may have unexpected results. You have been warned.</div><q>We fuse. We merge. We grow.</q>',640000000000,[6,9],function(){Game.elderWrath=2;Game.SetResearch('Arcane sugar');});
		...
		new Game.Upgrade('Elder Pact','[Research]<br>Each Ogge gains <b>+1 base KpS for each 20 portals</b>.<div class="warning">Note : this is a bad idea.</div><q>squirm crawl slither writhe<br>today we rise</q>',2560000000000,[8,9],function(){Game.elderWrath=3;});
		...
		order=150;
		new Game.Upgrade('Plastic mouse','Clicking gains <b>+1% of your KpS</b>.',50000,[11,0]);
		new Game.Upgrade('Iron mouse','Clicking gains <b>+1% of your KpS</b>.',5000000,[11,0]);
		new Game.Upgrade('Titanium mouse','Clicking gains <b>+1% of your KpS</b>.',500000000,[11,1]);
		new Game.Upgrade('Adamantium mouse','Clicking gains <b>+1% of your KpS</b>.',50000000000,[11,2]);
		...
		order=15000;
		new Game.Upgrade('Elder Covenant','[Switch]<br>Puts a permanent end to the elders\' wrath, at the price of 5% of your KpS.',6666666666666,[8,9],function()
		{
			Game.pledgeT=0;
		...
		new Game.Upgrade('Revoke Elder Covenant','[Switch]<br>You will get 5% of your KpS back, but the grandmatriarchs will return.',6666666666,[8,9],function()
		{
		...
		order=1000;
		new Game.Upgrade('Sugar bosons','Antimatter condensers gain <b>+99,999</b> base KpS.',Game.Objects['Starkeman'].basePrice*tier1,[13,0]);
		new Game.Upgrade('String theory','Antimatter condensers are <b>twice</b> as efficient.',Game.Objects['Starkeman'].basePrice*tier2,[13,0]);
		new Game.Upgrade('Large macaron collider','Antimatter condensers are <b>twice</b> as efficient.<q>How singular!</q>',Game.Objects['Starkeman'].basePrice*tier3,[13,1]);
		new Game.Upgrade('Big bang bake','Antimatter condensers are <b>twice</b> as efficient.<q>And that\'s how it all began.</q>',Game.Objects['Starkeman'].basePrice*tier4,[13,2]);

		order=250;
		new Game.Upgrade('AntiOgge','Ogge are <b>twice</b> as efficient.',Game.Objects['Starkeman'].basePrice*tier2,[10,9],function(){Game.Objects['Ogge'].drawFunction();});
		...
		order=20000;
		new Game.Upgrade('Kitten overseers','You gain <b>more KpS</b> the more milk you have.<q>my purrpose is to serve you, sir</q>',900000000000000,[8,7]);
		...
		*/
		...
		for (var i=0;i<Game.moneyAchievs.length/2;i++)
		{
			...
			if (i==15) pic=[11,5];
			new Game.Achievement(Game.moneyAchievs[i*2],'Make <b>'+Beautify(Game.moneyAchievs[i*2+1])+'</b> kart!'+(Game.moneyAchievs[i*2+1]==1?'':'s')+'.',pic,2);
		}
		...
		'A world filled with kart',	100000000,
		'Let\'s never bake again',		1000000000
		];
		for (var i=0;i<Game.cpsAchievs.length/2;i++)
		{
			var pic=[i,5];
			new Game.Achievement(Game.cpsAchievs[i*2],'Make <b>'+Beautify(Game.cpsAchievs[i*2+1])+'</b> kart'+(Game.cpsAchievs[i*2+1]==1?'':'s')+' per second.',pic,2);
		}
		
		order=30000;
		new Game.Achievement('Sacrifice','Reset your game with <b>1 million</b> kart baked.<q>Easy come, easy go.</q>',[11,6],2);
		new Game.Achievement('Oblivion','Reset your game with <b>1 billion</b> kart baked.<q>Back to square one.</q>',[11,6],2);
		new Game.Achievement('From scratch','Reset your game with <b>1 trillion</b> kart baked.<q>It\'s been fun.</q>',[11,6],2);
		
		order=31000;
		new Game.Achievement('Neverclick','Make <b>1 million</b> kart by only having clicked <b>15 times</b>.',[12,0],3);
		order=1000;
		new Game.Achievement('Clicktastic','Make <b>1,000</b> kart from clicking.',[11,0]);
		new Game.Achievement('Clickathlon','Make <b>100,000</b> kart from clicking.',[11,1]);
		new Game.Achievement('Clickolympics','Make <b>10,000,000</b> kart from clicking.',[11,1]);
		new Game.Achievement('Clickorama','Make <b>1,000,000,000</b> kart from clicking.',[11,2]);
		...
		order=1100;
		new Game.Achievement('Just wrong','Sell a Ogge.<q>I thought you loved me.</q>',[10,9],2);
		new Game.Achievement('Ogge\'s cookies','Have <b>1</b> grandma.',[1,0]);
		new Game.Achievement('Sloppy kisses','Have <b>50</b> Ogge.',[1,1]);
		new Game.Achievement('Retirement home','Have <b>100</b> Ogge.',[1,2]);
		
		order=1200;
		new Game.Achievement('nummmer 1 kommunism','<b>1</b> kommunistiskt samhällen., nice.',[2,0]);
		new Game.Achievement('Reap what you sow','Have <b>50</b> kommunistiska samhällen.',[2,1]);
		new Game.Achievement('Kommunistiska samhällen * 100','Have <b>100</b> kommunistiska samhällen.',[2,2]);
		...
		new Game.Achievement('Lucky cookie','Click <b>7 golden kart</b>.',[10,1],1);
		new Game.Achievement('A stroke of luck','Click <b>27 golden kart</b>.',[10,1],1);
		
		order=30200;
		new Game.Achievement('Cheated kart taste awful','Hack in some kart.',[10,6],3);
		order=30001;
		...
		order=10000;
		new Game.Achievement('Fortune','Click <b>77 golden kart</b>.<q>You should really go to bed.</q>',[10,1],1);
		order=31000;
		new Game.Achievement('True Neverclick','Make <b>1 million</b> kart with <b>no</b> cookie clicks.<q>This kinda defeats the whole purpose, doesn\'t it?</q>',[12,0],3);
		...
		order=1100;
		new Game.Achievement('Elder','Own every Ogge type.',[10,9],2);
		...
		order=10000;
		new Game.Achievement('Leprechaun','Click <b>777 golden kart</b>.',[10,1],1);
		new Game.Achievement('Black cat\'s paw','Click <b>7777 golden kart</b>.',[10,1],3);
		
		order=30000;
		new Game.Achievement('Nihilism','Reset your game with <b>1 quadrillion</b> kart made.<q>There are many things<br>that need to be erased</q>',[11,6],2);
		//new Game.Achievement('Galactus\' Reprimand','Reset your game with <b>1 quintillion</b> coo- okay no I'm yanking your chain
		
		order=1900;
		new Game.Achievement('Antibatter','Have <b>1</b> Starkeman.',[13,0]);
		new Game.Achievement('Quirky quarks','Have <b>50</b> Starkemans.',[13,1]);
		new Game.Achievement('It does matter!','Have <b>100</b> Starkemans.',[13,2]);
		...
		{
			if (Game.Has('Elder Covenant') || Game.Objects['Ogge'].amount==0) Game.elderWrath=0;
		...
		Game.DrawGrandmapocalypse=function()
		{
			...
			if (Math.abs((Game.elderWrath+1)-Game.elderWrathD)>0.1)
			{
				...
				else if (Game.elderWrathD>=1 && Game.elderWrathD<2)
				{
					...
					if (Game.bg!=Game.defaultBg || Game.bgFade!='Ogge1')
					{
						Game.bg=Game.defaultBg;
						Game.bgFade='Ogge1';
						l('backgroundLayer1').style.background='url(img/'+Game.bg+'.jpg)';
				...
				else if (Game.elderWrathD>=2 && Game.elderWrathD<3)
				{
					...
					if (Game.bg!='Ogge1' || Game.bgFade!='Ogge2')
					{
						Game.bg='Ogge1';
						Game.bgFade='Ogge2';
						l('backgroundLayer1').style.background='url(img/'+Game.bg+'.jpg)';
				...
				else if (Game.elderWrathD>=3 && Game.elderWrathD<4)
				{
					...
					if (Game.bg!='Ogge2' || Game.bgFade!='Ogge3')
					{
						Game.bg='Ogge2';
						Game.bgFade='Ogge3';
						l('backgroundLayer1').style.background='url(img/'+Game.bg+'.jpg)';
	...
	Game.Logic=function()
	{
		...
		if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
		{
			...
			//if (Game.Objects['Theo'].amount>=50 && Game.Objects['Theo'].specialUnlocked==0) {Game.Objects['Theo'].unlockSpecial();Game.Popup('You have unlocked the Theo dungeons!');}
			...
			if (!Game.HasAchiev('Elder') && Game.Has('Farmer Ogge') && Game.Has('Worker Ogge') && Game.Has('Miner Ogge') && Game.Has('Cosmic Ogge') && Game.Has('Transmuted Ogge') && Game.Has('Altered Ogge') && Game.Has('Grandmas\' Ogge')) Game.Win('Elder');
			if (Game.Objects['Ogge'].amount>=6 && !Game.Has('Bingo center/Research facility') && Game.HasAchiev('Elder')) Game.Unlock('Bingo center/Research facility');
		...
		if (Game.T%(Game.fps)==0) document.title=Beautify(Game.cookies)+' '+(Game.cookies==1?'kart':'kart')+' - Kart Clicker';
	...
	Game.Draw=function()
	{
		...
		var unit=(Math.round(Game.cookiesd)==1?' Kart':' Kart');
		if (Math.round(Game.cookiesd).toString().length>11) unit='<br>Kart';
		...
		*/
		...
		for (var i in Game.Objects)
		{
			...
			//update object info
			if (l('rowInfo'+me.id) && Game.T%5==0) l('rowInfoContent'+me.id).innerHTML='&bull; '+me.amount+' '+(me.amount==1?me.single:me.plural)+'<br>&bull; producing '+Beautify(me.storedTotalCps,1)+' '+(me.storedTotalCps==1?'kart':'kart')+' per second<br>&bull; total : '+Beautify(me.totalCookies)+' '+(Math.floor(me.totalCookies)==1?'kart':'kart')+' '+me.actionName;
		}
